    ...
    if ($_POST['loginActive'] == "0") {
        ...
        if (mysqli_num_rows($result) > 0) {
            ...
            if (mysqli_query($link, $query)) {
                ...
                $query = "UPDATE `users` SET password = '". md5(md5($_SESSION['id']).$_POST['password']). "' WHERE email = '". mysqli_real_escape_string($link, $_POST['email']). "' LIMIT 1";
        ...
        if ($row['password'] == md5(md5($row['id']).$_POST['password'])) {
            ...
            $error = "Couldn't find that username/password combination";
